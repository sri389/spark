...
object Joins {
  ...
  def nestedQuery(sqlcontext:SQLContext){
    //Nested query
    val nestedOutput = sqlcontext.sql("select customerId,total_amount " +
      "from ( select customerId, sum(amountPaid) total_amount from sales group by customerId ) t2 " +
      "where t2.total_amount >=600")
    nestedOutput.show()
